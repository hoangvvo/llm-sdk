...
// artifact_update
#[derive(Deserialize)]
struct ArtifactUpdateParams { id: String, content: String }

pub struct ArtifactUpdateTool;

#[async_trait]
impl AgentTool<MyContext> for ArtifactUpdateTool {
    fn name(&self) -> String { "artifact_update".to_string() }
    fn description(&self) -> String { "Replace document content and return an instruction for the client to persist changes".to_string() }
    fn parameters(&self) -> JSONSchema {
        serde_json::json!({
            "type": "object",
    ...
    async fn execute(
        ...
        let title = prev.map(|a| a.title.clone()).unwrap_or_else(|| "Untitled".to_string());
        let kind = prev.map(|a| a.kind.clone()).unwrap_or(ArtifactKind::Markdown);
        let prev_content = prev.map(|a| a.content.clone()).unwrap_or_default();
        let artifact = Artifact { id: params.id.clone(), title, kind, content: params.content, version: Some(next_version), updated_at: Some(now) };
...
// artifact_get
#[derive(Deserialize)]
struct ArtifactGetParams { id: String }
pub struct ArtifactGetTool;

#[async_trait]
impl AgentTool<MyContext> for ArtifactGetTool {
    fn name(&self) -> String { "artifact_get".to_string() }
    fn description(&self) -> String { "Fetch a document from the current client context".to_string() }
    fn parameters(&self) -> JSONSchema {
        serde_json::json!({
            "type": "object",
    ...
    async fn execute(
        ...
        let payload = serde_json::json!({"op": "artifact_get", "id": params.id, "artifact": artifact});
        Ok(AgentToolResult { content: vec![Part::text(payload.to_string())], is_error: false })
    }
}
...
impl AgentTool<MyContext> for ArtifactListTool {
    fn name(&self) -> String { "artifact_list".to_string() }
    fn description(&self) -> String { "List documents from the current client context".to_string() }
    fn parameters(&self) -> JSONSchema { serde_json::json!({"type": "object", "properties": {}, "additionalProperties": false}) }
    async fn execute(
        &self,
        _args: Value,
        context: &MyContext,
...
// artifact_delete
#[derive(Deserialize)]
struct ArtifactDeleteParams { id: String }
pub struct ArtifactDeleteTool;

#[async_trait]
impl AgentTool<MyContext> for ArtifactDeleteTool {
    fn name(&self) -> String { "artifact_delete".to_string() }
    fn description(&self) -> String { "Delete a document by id (client will persist)".to_string() }
    fn parameters(&self) -> JSONSchema {
        serde_json::json!({
            "type": "object",
            "properties": {"id": {"type": "string"}},
