...
<Aside>
  ...
  `AssistantMessage`. The nested `call.type` discriminates client function calls
  from provider-hosted web search without combining two discriminators at the
  same object level.
</Aside>
...
<Aside>
  `ToolResultPart.result` mirrors the call union. Function results contain an
  array of `Part`, enabling multimodal results; web-search results contain
