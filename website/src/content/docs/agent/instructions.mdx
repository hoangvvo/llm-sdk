...
<Aside type="caution">
  Dynamic instructions are resolved only once when the run session starts. They will not update again until you create a new session, so include per-turn data in the conversation or move it into a `ToolkitSession` that exposes `system_prompt` updates each turn.
</Aside>
