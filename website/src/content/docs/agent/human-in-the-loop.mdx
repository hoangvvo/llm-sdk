...
<Aside>
  This is an agent pattern that requires custom implementation.
  [Why?](/agent/#agent-patterns)
</Aside>
...
sequenceDiagram
    ...
    App-->>Human: deliver outcome
```

## Implementation

<Aside>
  At the moment, the only way to implement this pattern is to use `run_stream`,
  as it gives you access to the intermediate items. I am revising the API to
  make this easier for `run` in the future.
</Aside>
