...
<Aside type="caution">
  Despite the name "session", a run session does not persist `AgentItem`s between calls. Every `run` or `run_stream` starts fresh from the `input` you provide.
  If you want to continue a conversation, the `output` from one call must be appended to the `input` of the next.
</Aside>
