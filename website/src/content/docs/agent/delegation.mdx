...
1. The coordinate agent calls the suitable sub-agent via the `delegate_to_{agent}` tool. The tool takes the task string and rewrites it into a single `UserMessage`.
