...
<Aside type="danger">
  Avoid exposing immutable data as tool parameters. For example, if a parameter determines access (`current_user_id` or `account_id`), a malicious prompt or jailbreak might coerce the model into supplying a forged value. Instead, keep those values in the context object and read them inside `execute` so the model can never override them.
</Aside>
