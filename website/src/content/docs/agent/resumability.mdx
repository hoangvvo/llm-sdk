...
Runs across the agent libraries are deterministic so that a run can pick up exactly where it left off. The run logic can reprocess saved items, skip work that is already done, and only generate a model response when strictly necessary. This page explains how that works and how to take advantage of it.
...
- Human-in-the-loop approvals: pause after emitting a tool request, store the state, and resume only once the approval arrives.
