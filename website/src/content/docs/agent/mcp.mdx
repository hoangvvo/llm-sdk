...
<Aside type="note">
  Stdio transports spawn a child process (`uvx`, `python`, etc.) for the lifetime of the run session. Streamable HTTP transports keep a persistent connection open when the server supports it.
</Aside>
