...
<Aside type="note">
  Stdio transports spawn a child process (`npx`, `python`, etc.) for the
  lifetime of the run session. Streamable HTTP transports keep a persistent
  connection open when the server supports it. Make sure they shut down cleanly
  when the session ends to avoid resource leaks.
</Aside>
...
The examples below show how to connect an agent to an MCP server and call its tools.
