...
use llm_sdk::{
    FunctionTool, LanguageModelInput, Message, Part, StreamAccumulator, Tool, ToolCall, ToolResult,
    ToolResultPart, ToolSearchTool,
};
...
async fn main() {
    ...
    for _ in 0..10 {
        ...
        let mut stream = model
            .stream(LanguageModelInput::new(messages.clone()).with_tools(tools.clone()))
            .await
        ...
        while let Some(partial) = stream.next().await {
            ...
            accumulator
                .add_partial(partial)
                .expect("Failed to add partial");
        }

        // Execute function calls only after their streamed arguments are
        // complete.
